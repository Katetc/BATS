...
run_parser.add_argument(
    "--scale", type=unsigned_int, default=0, help="Change the degrees of freedom."
)
run_parser.add_argument(
    "--sizes", type=unsigned_int, metavar="KM", help="Change the domain size."
)
...
def case_run_directory(case_dir, run_args):
    case_run_dir = os.path.join(
        case_dir, "s" + str(run_args.scale), "p" + str(run_args.parallel)
    )
...
def mkdir_test(args, test_dict):
    ...
    for case in test_dict:
        ...
        case_dir = os.path.normpath(
            os.path.join(data_dir, str.split(case_split[0], "/")[-1], case_split[-1])
        )
...
def cmake(args):
    ...
    for i in reversed(range(len(spec) + 1)):
        ...
        if os.path.isdir(c_dir) and os.path.isfile(
            os.path.join(c_dir, args.cmake_file)
        ):
