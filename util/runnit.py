...
def personal(args, cism_driver, data_dir, test_dict):
    ...
    for case in test_dict:
        ...
        # run default test
        test_commands = [
            f"cd {Path('tests', case_split[0]).absolute()}",
            f"python3 {run_script} -q -e {cism_driver} -o {case_run_dir} {mod_arg} -n 1",
            "exit",
        ]

        print("   Spawning " + case + " test default...")
        test_run[case] = subprocess.Popen(
            " ; ".join(test_commands), executable="/bin/bash", shell=True
        )
        ...
        if args.performance and mod_dict:
            for mod in mod_dict:
                ...
                test_commands = [
                    f"cd {Path('tests', case_split[0]).absolute()} ",
                    f"python3 {run_script} -q -e {cism_driver} -o {case_run_dir} {mod_arg} {mod_dict[mod]}",
                    "exit",
                ]
...
def hpc(args, cism_driver, data_dir, test_dict):
    ...
    for case in test_dict:
        ...
        test_commands = [
            ...
            (
                f"python3 {run_script} -q -e {cism_driver} "
                f"-o {case_run_dir} {mod_arg} -s -n 1 --hpc {run_cmd}"
            ),
            "exit",
        ]
            ...
            for rnd in range(10):
                ...
                timing_commands.extend(
                    [
                        ...
                        (
                            f"python3 {run_script} -q -e {cism_driver} "
                            f"-o {case_run_dir} {timing_mod} -s -n 1 --hpc {run_cmd}"
                        ),
                    ]
                )
                ...
                for mod in mod_dict:
                    ...
                    timing_commands.extend(
                        [
                            ...
                            (
                                f"python3 {run_script} -q -e {cism_driver} "
                                f"-o {case_run_dir} {timing_mod} {mod_dict[mod]} -s "
                                f"--hpc {run_cmd}"
                            ),
                        ]
                    )
        ...
        if mod_dict:
            for mod in mod_dict:
                ...
                test_commands = [
                    ...
                    f"export PYTHONPATH=$PYTHONPATH:{cism_test_dir}",
                    (
                        f"python3 {run_script} -q -e {cism_driver} -o {case_run_dir} "
                        f"{mod_arg} {mod_dict[mod]} -s --hpc {run_cmd}"
                    ),
                    "exit",
                ]
                ...
                for mod in mod_dict:
                    ...
                    large_timing_commands.extend(
                        [
                            ...
                            (
                                f"python3 {run_script} -q -e {cism_driver} "
                                f"-o {case_run_dir} {timing_mod} {mod_dict[mod]} "
                                f"-s -n 1 --hpc {run_cmd}"
                            ),
                        ]
                    )
    ...
    large_run_files = list(
        set(small_run_files) ^ set(all_run_files)
    ...
    if args.timing:
        ...
        subprocess.check_call(
            str.join(" ; ", timing_commands), executable="/bin/bash", shell=True
        )
        ...
        for rnd in range(10):
            ...
            create_job(
                args, small_timing_job_name, platform_dict, small_timing_run_commands
            )
        ...
        subprocess.check_call(
            str.join(" ; ", large_timing_commands), executable="/bin/bash", shell=True
        )
        ...
        for rnd in range(10):
            ...
            create_job(
                args, large_timing_job_name, platform_dict, large_timing_run_commands
            )
